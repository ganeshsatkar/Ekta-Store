...
public class DotsAdapter extends RecyclerView.Adapter<DotViewHolder> {
    ...
    public DotViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(context).inflate(grdp.emart.store.R.layout.dots_list_items, null);
    ...
    public void onBindViewHolder(DotViewHolder holder, int position) {
        if (position == selectedPos) {
            holder.dotImageView.setColorFilter(ContextCompat.getColor(context, grdp.emart.store.R.color.colorPrimary));
        } else {
            holder.dotImageView.setColorFilter(ContextCompat.getColor(context, grdp.emart.store.R.color.gray));
        }
